...
structure ConeMorphism (A B : Cone F) where
  ...
  w : ∀ j : J, hom ≫ B.π.app j = A.π.app j := by sorry
...
@[target] theorem ConeMorphism.ext {c c' : Cone F} (f g : c ⟶ c') (w : f.hom = g.hom) : f = g := by
  sorry
...
Added nolint. -/
@[target] theorem Cone.w {F : J ⥤ C} (c : Cone F) {j j' : J} (f : j ⟶ j') :
